...
use File::Copy;
if (@ARGV<1)  {
	die "\nUsage: $0 [*-fil.hssp] [*-fil.rdbProf]\n";
	}
$hsspfil=$ARGV[0];$prof=$ARGV[1];
$fileroot=$hsspfil;
@orla=split('/',$fileroot) ;
$mcf=$orla[$#orla];
if ($mcf=~ /(.*)-fil\.hssp/) {
...
undef @M;undef @N;undef @P;undef @Q;undef @R;undef @S;undef @T;undef @V;undef @W;undef @Y;undef @meida;
 undef @NOCC;undef @NDEL;undef @EN;undef @RE;undef @WEI;
if (!open(FILE, "$prof"))  {
	die "cant open $prof\n";
	}		
...
while ($line=<FILE>)  {
	...
	$NOCC[$s]=substr($line, 92,6);$NOCC[$s]=~ s/\s//g;
	$NDEL[$s]=substr($line, 98,5);$NDEL[$s]=~ s/\s//g;
	$NINS[$s]=substr($line, 103,5);$NINS[$s]=~ s/\s//g;
	$EN[$s]=substr($line, 108,8);$EN[$s]=~ s/\s//g;
	$RE[$s]=substr($line, 116,7);$RE[$s]=~ s/\s//g;
	$WEI[$s]=substr($line, 123,7);$WEI[$s]=~ s/\s//g;
	$s++;
	}
...
open (FOUT, ">/home/schles/profbval/work/$dataFile") || print "cant open file $!";
print FOUT "num V L I M F W Y G A P S T C H R K Q E N D SEC ACC EXPCON EXPCON2 HCON BCON LCON L1 L2 L3 DISO WEI\n";
...
#	push (@info, $Helix, $Beta, $Loop);
	...
	push (@info,$DISO);push (@info,$res[$i]);push (@info, $WEI[$i]);
	print FOUT "$resNum[$i] ";		
	presentIt(\@info);
	}
close (FOUT);
