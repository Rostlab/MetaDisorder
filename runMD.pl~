...
$profcon_file="$work_dir/$id.profcon";
$ucon_only_file="$work_dir/".$id.".ucon_only";
$ucon_only_prob05_file="$work_dir/".$id.".ucon_only_prob05";
...
$IN=$win*21+9+4*$win_acc+4*$win_sec+21+4+3+2+3;@aa=('A','C','D','E','F','G','H','I','K','L','M','N','P','Q','R','S','T','V','W','Y');
...
$jct="$root/jct110-nu-w31-set5-ac1-sec1-p6f";
$jct="$root/jct40-win31-set5-acc1-sec1-prop6f"; # until I figure out which one to put here. May 2008
...
#	push (@info,@compo_all); ## 20
	...
	#push (@info,${$ucon_random{$id}}[$i]); ##### when PROFcon file is not available
...
sub print_help {
	...
	printf "%5s %-15s=%-20s %-s\n","","profcon", "x",           "takes profcon file; processing it to ucon is done locally ";
...
loop57: while ($line=<FILE>)  {
		...
		if ($moogla[3]>=0.60) { ####XXXXXXXXX make sure with ucon server
			push (@{$ucon2_states{$id}}, 'D');
			}
		else {
			 push (@{$ucon2_states{$id}}, '-');
			}
                }
        close(FILE);
        }
...
#               print FOUT "num\taa\tnorsnet\tnorsnet\tdiso\tdiso\tbval1\tbval2\tucon\tnode1\tnode2\tmd\tmd\n";
       ...
       #### the following erased on 10/23/2008 
	#elsif($mode_out==1) {
        #        print FOUT "num\taa\tNORSnet\tNORSnet2st\tPROFbval\tPROFbval2st\tUcon\tUcon2st\tMD\tMD2st\n";
        #       for ($i=0;$i<scalar@res;$i++) {
        #                $num=$i+1;
        #               printf FOUT "%4d\t%s\t%1.2f\t%s\t%1.2f\t%s\t%1.2f\t%s\t%1.3f\t%s\n",$num,$res[$i],${$nors_raw{$id}}[$i],${$nors2states{$id}}[$i],${$profbval_prob{$id}}[$i], ${$profbval2_states{$id}}[$i],${$ucon_prob{$id}}[$i],${$ucon2_states{$id}}[$i],$pred[$i],${$md2states{$id}}[$i];
        #                }
        #        close(FOUT);
        #        print "raw results are in $validOutFile\n";
        #        open (LOG, ">>$log_file") || die "cant open log file $!";
...
sub get_rel {
	...
	$iter{1}=229;	$iter{2}=77;	$iter{3}=113;	$iter{4}=159;	$iter{5}=40;$iter{6}=44; # this one is the method with ucon
       # $iter{1}=696; $iter{2}=68; $iter{3}=106; $iter{4}=84; $iter{5}=110; $iter{6}=37; #this one is the noUcon method
#        $min{1}=0.14;$max{1}=0.92; $min{2}=0.24;$max{2}=0.72; $min{3}=0.22;$max{3}=0.76;  $min{4}=0.26;$max{4}=0.76; $min{5}=0.22;$max{5}=0.76; $min{6}=0.30;$max{6}=0.72; #this one is the no Ucon method
        $min{1}=0.101;$max{1}=0.798; $min{2}=0.101;$max{2}=0.788; $min{3}=0.101;;$max{3}=0.798;  $min{4}=0.101;;$max{4}=0.798; $min{5}=0.101;;$max{5}=0.798; $min{6}=0.112;$max{6}=0.778; #this one is the method with ucon
